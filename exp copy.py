...
class Exp:
    def __init__(self):
        ...
        self.data_path = '数据/Water/Aquifer_Petrignano.csv'
        self.timestep = 30  # 时间步长，就是利用多少时间窗口
        ...
        self.save_path = 'result/model/{}.pth'.format(self.model_name)  # 最优模型保存路径
        self.plotsize = 500
    ...
    def dataload(self):
        ...
        data_loaders = []
        for x, y in data_tensors:
            x_tensor = torch.from_numpy(x).to(torch.float32).to(self.device)
            y_tensor = torch.from_numpy(y).to(torch.float32).to(self.device)
            data_set = TensorDataset(x_tensor, y_tensor)
            data_loader = DataLoader(data_set, self.batch_size, False)
            data_loaders.append(data_loader)
        self.x_train_tensor,= x_train_tensor
        self.y_train_tensor = y_train_tensor
        self.x_test_tensor = x_test_tensor
        self.y_test_tensor = y_test_tensor
        train_loader, vali_loader, test_loader = data_loaders
    ...
    def setmodel(self, modelname):
        ...
        if modelname in model_params:
            ...
            self.save_path = 'result/model/{}.pth'.format(self.model_name)
        else:
    ...
    def paint(self):
        ...
        # 均方误差
        sme = mean_squared_error(y_test_actual[:, -1], y_test_pred[:, -1], squared=False)
        # 均方根误差
        ...
        #保存绘图
        if not os.path.exists('result/pic/%s %.3f %.3f %.3f.png' % (self.model_name, sme, mape.item(), r_squared)):
            plt.savefig('result/pic/%s %.3f %.3f %.3f.png' % (self.model_name, sme, mape.item(), r_squared))

        else:
            plt.savefig('result/pic/+1%s %.3f %.3f %.3f.png' % (self.model_name, sme, mape.item(), r_squared))

        y_test = [np.array(y_test_actual), y_test_pred]

        #保存loss值和预测值
        if not os.path.exists('result/pre&loss/%s %.3f.npy' % (self.model_name, r_squared)):
            np.save('result/pre&loss/%s %.3f.npy' % (self.model_name, r_squared), y_test)
            np.save('result/loss/%s %.3f.npy' % (self.model_name, r_squared), [train_losses, val_losses, test_losses])
        else:
            np.save('result/pre&loss/+1%s %.3f.npy' % (self.model_name, r_squared), y_test)
            np.save('result/loss/+1%s %.3f.npy' % (self.model_name, r_squared), [train_losses, val_losses, test_losses])
